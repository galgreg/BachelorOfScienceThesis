...
class TestAgentsPopulation(unittest.TestCase):
	...
	@unpack
	@data((1, [5, 2], 0), (10, [5, 3, 2], 5), (100, [5, 20, 30, 7], 68))
	def test_SetDone(self, numberOfAgents, agentsDimensions, agentIndex):
		population = AgentsPopulation(numberOfAgents, agentsDimensions)
		agentToOperateOn = population._agents[agentIndex]
		
		doneValueBeforeCall = agentToOperateOn.IsDone()
		self.assertFalse(doneValueBeforeCall)
		
		population.SetDone(agentIndex)
		
		doneValueAfterCall = agentToOperateOn.IsDone()
		self.assertTrue(doneValueAfterCall)
		
	@unpack
	@data((1, [5, 2]), (10, [5, 3, 2]), (100, [5, 20, 30, 7]))
	def test_ResetAll(self, numberOfAgents, agentsDimensions):
		population = AgentsPopulation(numberOfAgents, agentsDimensions)
		...
		for agent in population._agents:
			doneValueBeforeCall = agent.IsDone()
			self.assertTrue(doneValueBeforeCall)

		population.ResetAll()
		
		for agent in population._agents:
			doneValueAfterCall = agent.IsDone()
			self.assertFalse(doneValueAfterCall)
